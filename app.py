from flask import Flask,redirect,url_for,render_template,request,session,Response,send_file
import json
...
def checkBalance():
    ...
    for i in users:
        if i["id"] == session['id']:
            balance = i["balance"]
            username = session.get('username', i['username'])
            user_pin = i.get('pin')
            if request.method == 'POST':
                entered_pin = request.form.get('pin')
                if entered_pin == user_pin:
                    return render_template('checkbalance.html', username=username, balance=balance, verified=True, pin_hint=user_pin)
                else:
                    return render_template('checkbalance.html', username=username, balance=balance, verified=False, info="Incorrect PIN! Please enter your valid ATM PIN.", pin_hint=user_pin)
            return render_template('checkbalance.html', username=username, balance=balance, verified=False, pin_hint=user_pin)
    return redirect(url_for('login'))
...
def deposit():
   if request.method=='POST':
        try:
            ...
            return render_template("deposit.html",username=session.get('username'),info="Enter a valid positive amount")
        data = get_data()
        users = data["users"]
        for i in users:
            if i["id"] == session["id"]:
   ...
   return render_template("deposit.html",username=session.get('username'))
...
def withdraw():
    if request.method == 'POST':
        try:
            ...
            return render_template("withdraw.html",username=session.get('username'),info="Enter the proper amount")
        ...
        for i in users:
            if i["id"] == session["id"]:
                if i["balance"] >= amount:
                    ...
                    return render_template("withdraw.html",username=session.get('username'),info="Insufficient balance")
...
def viewTransactions():
    ...
    for i in users:
        if i["id"] == session["id"]:
            history=i["history"]
            length=len(history)
            return render_template("viewtransactions.html",history=history,length=length,username=session.get('username'))
    return redirect(url_for('login'))
